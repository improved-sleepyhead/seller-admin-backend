...
Этот документ предназначен только для frontend-команды. Он описывает, как backend работает с точки зрения клиента: какие endpoint'ы доступны, какие payload'ы нужно отправлять, какие DTO приходят в ответ, как устроены ошибки и как правильно интегрировать AI-сценарии.
...
- [Backend_PRD_detailed.md](./Backend_PRD_detailed.md) — продуктовые требования
...
3. Принимает полное обновление объявления через `PUT`.
...
- Для `PUT` и `POST` отправляйте `Content-Type: application/json`.
...
Для `PUT /items/:id` и всех AI endpoint'ов используется полный write-side shape:
...
- `PUT /items/:id` не поддерживает partial update;
...
```ts
type PutItemRequest = ItemUpdateIn;
```
...
- Это не patch-endpoint.
- Нельзя отправлять только изменённые поля.
- Нужно отправлять весь объект объявления в write-side форме.
- После успешного `PUT` стоит перечитать объявление через `GET /items/:id`, если UI зависит от нового `updatedAt`.
...
### Пример общего type guard для axios/fetch
...
- Не рассчитывайте на partial update в `PUT /items/:id`.
